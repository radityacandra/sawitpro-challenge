package handler

import (
	"encoding/json"
	"errors"
	"fmt"
	"net/http"
	"net/http/httptest"
	"strings"
	"testing"
	"time"

	"github.com/SawitProRecruitment/UserService/generated"
	"github.com/SawitProRecruitment/UserService/model"
	"github.com/SawitProRecruitment/UserService/pkg/jwt"
	"github.com/SawitProRecruitment/UserService/repository"
	"github.com/golang/mock/gomock"
	"github.com/labstack/echo/v4"
	"github.com/stretchr/testify/assert"
)

func TestUpdateProfile(t *testing.T) {
	t.Setenv("JWT_PRIVATE_KEY", `
-----BEGIN RSA PRIVATE KEY-----

f9ssIXGETX3SAASpToGeHh/iqcMcOahE6K7niu47lMqAbwvmNugk
-----END RSA PRIVATE KEY-----
	`)

	t.Setenv("JWT_PUBLIC_KEY", `
-----BEGIN PUBLIC KEY-----

-----END PUBLIC KEY-----
	`)

	type testCase struct {
		name                 string
		reqBody              string
		tokenUserId          int
		tokenExpiredAt       int64
		jwtToken             bool
		getUserById          *model.User
		getUserByPhoneNumber *model.User
		updateUser           error
		expectStatusCode     int
	}

	user := model.NewUser("Test", "+6281123456789")
	user.SetupPassword("Password123!!!")

	otherUser := model.NewUser("Test2", "+6281123456788")
	user.SetupPassword("Password123!!!")

	testCases := []testCase{
		{
			name:                 "should return error if doesn't provided authorization token",
			reqBody:              `{}`,
			tokenUserId:          5,
			tokenExpiredAt:       time.Now().Add(3 * time.Hour).Unix(),
			jwtToken:             false,
			getUserById:          nil,
			getUserByPhoneNumber: nil,
			updateUser:           nil,
			expectStatusCode:     403,
		},
		{
			name:                 "should return error if failed to parse req body",
			reqBody:              `{"fullName": 123}`,
			tokenUserId:          5,
			tokenExpiredAt:       time.Now().Add(3 * time.Hour).Unix(),
			jwtToken:             true,
			getUserById:          nil,
			getUserByPhoneNumber: nil,
			updateUser:           nil,
			expectStatusCode:     422,
		},
		{
			name:                 "should return validation error",
			reqBody:              `{"phoneNumber":"6281123456789"}`,
			tokenUserId:          5,
			tokenExpiredAt:       time.Now().Add(3 * time.Hour).Unix(),
			jwtToken:             true,
			getUserById:          nil,
			getUserByPhoneNumber: nil,
			updateUser:           nil,
			expectStatusCode:     400,
		},
		{
			name:                 "should return error if there is conflicting phone number",
			reqBody:              `{"phoneNumber":"+6281123456788"}`,
			tokenUserId:          5,
			tokenExpiredAt:       time.Now().Add(3 * time.Hour).Unix(),
			jwtToken:             true,
			getUserById:          user,
			getUserByPhoneNumber: otherUser,
			updateUser:           nil,
			expectStatusCode:     409,
		},
		{
			name:                 "should return error if failed to update data",
			reqBody:              `{"phoneNumber":"+6281123456788"}`,
			tokenUserId:          5,
			tokenExpiredAt:       time.Now().Add(3 * time.Hour).Unix(),
			jwtToken:             true,
			getUserById:          user,
			getUserByPhoneNumber: nil,
			updateUser:           errors.New("some error"),
			expectStatusCode:     500,
		},
		{
			name:                 "should return success",
			reqBody:              `{"phoneNumber":"+6281123456788"}`,
			tokenUserId:          5,
			tokenExpiredAt:       time.Now().Add(3 * time.Hour).Unix(),
			jwtToken:             true,
			getUserById:          user,
			getUserByPhoneNumber: nil,
			updateUser:           nil,
			expectStatusCode:     202,
		},
	}
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			var reqBody generated.UserProfileDto
			json.Unmarshal([]byte(testCase.reqBody), &reqBody)

			token, _, _ := jwt.BuildToken(map[string]interface{}{
				"userId": testCase.tokenUserId,
			})

			if !testCase.jwtToken {
				token = ""
			}

			ctrl := gomock.NewController(t)
			defer ctrl.Finish()

			m := repository.NewMockRepositoryInterface(ctrl)
			m.EXPECT().GetUserById(gomock.Any(), testCase.tokenUserId).Return(testCase.getUserById).AnyTimes()

			if reqBody.PhoneNumber != nil {
				m.EXPECT().GetUserByPhoneNumber(gomock.Any(), *reqBody.PhoneNumber).Return(testCase.getUserByPhoneNumber).AnyTimes()
			}

			m.EXPECT().UpdateUser(gomock.Any(), gomock.Any()).Return(testCase.updateUser).AnyTimes()

			server := NewServer(NewServerOptions{
				Repository: m,
			})

			e := echo.New()
			generated.RegisterHandlers(e, server)

			req := httptest.NewRequest(http.MethodPost, "/users/profile", strings.NewReader(testCase.reqBody))
			req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
			req.Header.Set(echo.HeaderAuthorization, fmt.Sprintf("Bearer %s", token))
			rec := httptest.NewRecorder()
			c := e.NewContext(req, rec)

			assert.NoError(t, server.UpdateProfile(c))
			assert.Equal(t, testCase.expectStatusCode, rec.Code)
		})
	}
}
